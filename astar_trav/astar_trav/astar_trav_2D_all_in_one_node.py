...
class AstarImageSnapshot:
    def __init__(self):
        self.raw_color_img = None
        self.raw_depth_img = None
        self.predicted_mask_labels = None # Predicted mask labels
        self.predicted_seg_img = None # Semseg image from predicted labels
        self.stamp = None # builtin_interfaces.Time message


class AstarMapSnapshot:
    def __init__(self):
        self.obstacle_votes = None # 1D numpy array of how many obstacle votes are at each obstacle/minor vertex
        self.minor_vertices = None # 2D numpy array of the (x,y) locations for each obstacle/minor vertex
        self.max_votes_per_vertex = None
        self.opt_path = None
        self.opt_cost = None
        self.approx_tracking_waypoint = None

        self.direct_path = None
        self.direct_path_cost = None
...
class AstarPathSnapshot:
    """Keep a list of all relevant astar path data as they appear. Each index corresponds to a single time instance."""
    def __init__(self):
        self.vehicle_position = None
        self.vehicle_orientation = None
...
class AstarSnapshotsSummary:
    def __init__(self):
        self.vehicle_enabled_duration = None
        self.active_camera_processing_duration = None
        self.expected_camera_frame_rate = None
        self.num_expected_camera_images = None
        self.image_processing_duration : List[float] = [] # List of the durations for processing each pair of images
        self.num_images_processed = None

        self.active_path_replanning_duration = None
        self.expected_path_replanning_rate = None
        self.num_expected_replanning_steps = None
        self.num_replanning_steps = None
        self.astar_iterations_per_replanning_step : List[int] = []
        self.astar_replanning_step_duration : List[float] = [] # List of durations for each A* replanning step


class AstarTrav2DAllInOne(AutoSceneGenVehicleNode):
    PLOT_PADDING = 5.
    ...
    def __init__(self, 
                ...
                b_use_scene_description: True,
                ssa_to_ignore: List[str],
        ...
        # Debugging params to bypass perception. 
        # Here, we will use the scene description and generate a single path from start to goal.
        self.b_use_scene_description = b_use_scene_description
        self.ssa_to_ignore = ssa_to_ignore
        self.b_sent_global_astar_path = False
        ...
        self.log("info", f'Initialized A* trav path planner for worker ID {self.wid}.')
    ...
    def reset(self):
        ...
        self.astar_proc_time = 0.
        self.num_waypoint_msgs_sent = 0
        self.log("info", f"All-in-one path planner has been reset.")
    ...
    def update_map(self, color_img: np.float32, depth_img: np.float32, img_stamp: builtin_interfaces.Time):
        ...
        with self.lock:
            if not self.b_use_scene_description:
                ...
                if not self.b_sent_global_astar_path:
                    self.path_planner.clear_map()
                    for layout in self.scene_description.ssa_array:
                        b_ignore = False
                        for ssa_to_ignore in self.ssa_to_ignore:
                            if ssa_to_ignore in layout.path_name:
                                b_ignore = True
                                break
                        ...
                        if not b_ignore:
                            for i in range(layout.num_instances):
                                if layout.visible[i]:
                                    obs_xy = np.array([layout.x[i], layout.y[i]], dtype=np.float32)
                                    self.path_planner.add_obstacle_vote(obs_xy, votes_to_add = 50)
        ...
        if not self.b_use_scene_description:
            if self.replan_path_timer is None and stamp_utils.get_stamp_dt(self.initial_cam_proc_stamp, self.sim_clock.now().to_msg()) >= self.initial_sensor_proc_delay:
            # if self.replan_path_timer.is_canceled() and stamp_utils.get_stamp_dt(self.initial_cam_proc_stamp, self.sim_clock.now().to_msg()) >= self.initial_sensor_proc_delay:
                self.initial_astar_replan_stamp = self.sim_clock.now().to_msg()

                # Create/reset timer
                self.log("info", "Creating update path timer")
            ...
            if not self.b_sent_global_astar_path:
                self.b_sent_global_astar_path = self.replan_path()

    def replan_path(self):
        """This is the main piece of code for the A* traversability path planner"""
        if not self.vehicle_ok() or self.goal_point is None or self.vehicle_position is None or self.img_stamp is None:
            return

        with self.lock:
            path_stamp = self.map_snapshots[-1].stamp
            obstacle_votes = self.path_planner.obstacle_votes.copy()
        ...
        if self.b_use_yaw_radius_threasholds:
            opt_path, opt_cost, iterations = self.path_planner.construct_optimal_path(start_vertex, goal_point, start_yaw=yaw, event=self.vehicle_disabled_event)
        else:
        ...
        path_snapshot.obstacle_votes = obstacle_votes
        ...
        with self.lock:
            if len(self.map_snapshots) > 0:
                self.map_snapshots[-1].opt_path = opt_path
                self.map_snapshots[-1].opt_cost = opt_cost
                self.map_snapshots[-1].obstacle_votes = obstacle_votes
                self.map_snapshots[-1].direct_path = direct_path
                self.map_snapshots[-1].direct_path_cost = direct_cost
    ...
    def save_path_planner_video(self):
        ...
        for i in range(len(self.map_snapshots)):
            ...
            # fig.legend(handles, labels, loc='lower center', ncol=3, bbox_to_anchor=(.5, 1), fontsize=12)
    ...
    def save_node_data(self):
        ...
        self.log("info", f"PROCESSED {len(self.map_snapshots)} snapshots in {self.image_proc_time:.4f} sec (avg rate {self.image_proc_time/len(self.map_snapshots):.4f} sec/snapshot) over {camera_dt:.2f} seconds of sensor data")
...
def main(args=None):
    ...
    path_planner_dict = {
        ...
        "minor_cell_size": 1.,
        "major_cell_size": 2.,
        "bell_curve_sigma": 2.5, # 2.5 in v1, 1.5 in v2
    ...
    astar_trav_dict = {
        ...
        "b_use_vectorized_obstacle_vote_update": False,
        "b_use_multithreaded_path_update": True,
        "b_use_yaw_radius_threasholds": False,
        "b_use_scene_description": False,
        "ssa_to_ignore": [], # SSA_Tree
    ...
    node = AstarTrav2DAllInOne(node_name, semseg_dict, path_planner_dict, **astar_trav_dict)
